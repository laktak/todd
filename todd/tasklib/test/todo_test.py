...
@pytest.fixture
def tasklist():
    return Tasklist([TODO_COWS, TODO_FLUX, TODO_TRASH, TODO_DONE, TODO_PLAN])
...
def test_tasklist_sorted(tasklist):
    tasklist.set_text_items([TODO_FLUX, TODO_COWS, TODO_TRASH, TODO_DONE, TODO_PLAN])
    assert [task.task_id for task in tasklist.get_items()] == [6, 7, 8, 9, 10]

    assert [task.task_id for task in tasklist.get_items_sorted("due")] == [8, 6, 10, 7, 9]

    assert [task.task_id for task in tasklist.get_items_sorted("prio")] == [6, 8, 10, 7, 9]


def test_tasklist_filter_context(tasklist):
    assert [t.task_id for t in Tasklist.filter_context(tasklist.get_items(), "@weekend")] == [2, 5]


def test_tasklist_set_items(tasklist):
    tasklist.set_text_items([TODO_FLUX, TODO_TRASH, TODO_DONE, "2000-01-02 " + TODO_PLAN])
...
def test_task_undo(tasklist):
    tasklist.set_text_items([TODO_TRASH, TODO_DONE, TODO_PLAN])
...
def test_task_is_done(tasklist):
    tasklist.set_text_items([TODO_COWS, TODO_TRASH, TODO_DONE])
    assert [t.is_done() for t in tasklist] == [False, False, True]
...
def test_tasklist_delete(tasklist):
    ...
    assert [t.raw for t in tasklist] == [TODO_FLUX, TODO_TRASH, TODO_DONE, TODO_PLAN]
    assert [task.task_id for task in tasklist.get_items()] == [2, 3, 4, 5]
    tasklist.delete_by_id(5)
    assert [t.raw for t in tasklist] == [TODO_FLUX, TODO_TRASH, TODO_DONE]
...
def test_tasklist_search(tasklist):
    ...
    assert [t.raw for t in Tasklist.search(search, tasklist.get_items())] == [TODO_FLUX, TODO_DONE]
